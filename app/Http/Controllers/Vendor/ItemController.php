...
class ItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (Helpers::get_vendor_module_id() == 'services') {
            ...
            $service->store_id = Helpers::get_store_data()->id;
            ...
            $food->category_id = $request->sub_sub_category_id ? $request->sub_sub_category_id : ($request->sub_category_id ? $request->sub_category_id : $request->category_id);
    ...
    public function get_items(Request $request)
    {
        ...
        $cat = DB::table('items')->where('category_id', $request->parent_id)->select('id', 'name')->get();
        // dd($cat);
        $res = '<option value="' . 0 . '" disabled selected>---Select---</option>';
        foreach ($cat as $row) {
            // if ($row->id == $request->sub_category) {
    ...
    public function pending_list(Request $request){
        ...
        // dd($items);
        $category = $category_id != 'all' ? Category::findOrFail($category_id) : null;
        return view('vendor-views.product.pending-list', compact('items', 'category', 'type', 'sub_categories'));
    }
